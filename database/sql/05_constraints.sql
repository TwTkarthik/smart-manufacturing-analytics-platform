...
-- Naming: fk_{child_table}_{parent_table}
-- ─────────────────────────────────────────────────────────────────────────────

-- machines.line_code → production_lines.line_code
ALTER TABLE machines
    ADD CONSTRAINT fk_machines_production_lines
    FOREIGN KEY (line_code) REFERENCES production_lines (line_code);

-- employees.shift_assignment → shifts.shift_code
...
-- production_orders → employees (nullable: operator may be NULL for automated)
ALTER TABLE production_orders
...
-- downtime_events → employees (nullable: reporter may be NULL)
ALTER TABLE downtime_events
...
-- maintenance_logs → employees (nullable: technician may be unassigned)
ALTER TABLE maintenance_logs
    ADD CONSTRAINT fk_maintenance_logs_employees
    FOREIGN KEY (technician_id) REFERENCES employees (employee_id);

-- maintenance_logs → pm_schedules (nullable: NULL for unplanned events)
ALTER TABLE maintenance_logs
...
-- material_movements → maintenance_logs (nullable: NULL for receipts/transfers)
ALTER TABLE material_movements
...
-- Note: Applied on parent table — applies to all partitions automatically.
...
ALTER TABLE shifts
    ...
    CHECK (shift_duration_hours > 0 AND planned_production_hours > 0
           AND planned_production_hours <= shift_duration_hours);

-- machines
ALTER TABLE machines
    ADD CONSTRAINT chk_machines_machine_type_code
    CHECK (machine_type_code IN ('MCH-LATHE', 'MCH-MILL', 'MCH-GRIND',
...
-- Ensure actual_end >= actual_start when both are present
ALTER TABLE production_orders
...
ALTER TABLE sensor_readings
    ...
    CHECK (sensor_type IN ('temperature', 'vibration', 'rpm', 'pressure',
...
ALTER TABLE quality_inspections
    ...
    CHECK (inspection_type_code IN ('FIRST-ARTICLE', 'IN-PROCESS', 'FINAL', 'FUNCTIONAL'));
...
ALTER TABLE defect_types
    ...
    CHECK (defect_category IN ('Dimensional', 'Surface', 'Structural', 'Functional', 'Other'));
...
ALTER TABLE spare_parts
    ...
    CHECK (part_category IS NULL OR
           part_category IN ('Bearings', 'Seals', 'Filters', 'Belts',
...
ALTER TABLE material_movements
    ...
    CHECK (movement_type IN ('GOODS_ISSUE', 'GOODS_RECEIPT', 'STOCK_TRANSFER', 'RETURN'));
...
-- machines: SCADA tag names and asset tags must be globally unique
ALTER TABLE machines
...
-- production_orders: ERP order ID must be unique where present
ALTER TABLE production_orders
